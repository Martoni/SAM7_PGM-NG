...
static int read_serial_port_timeout_nb(unsigned char *buf, int size, const char *term, double timeout)
{
	int r, num=0;
	long sec, usec;
	double ipart;
	...
	usec = (long)(modf(timeout, &ipart) * 1000000.0);
	sec = (long)(ipart);
	//printf("timeout=%.2f, sec=%ld, usec=%ld\n", timeout, sec, usec);

	while (1) {
		//printf("read/timeout loop\n");
		// try to read (nonblocking)
		r = read(port_fd, buf + num, size - num);
		//printf("read/timeout loop 2\n");
		...
		if (r > 0) {
			...
			if (term != NULL) {
				buf[num] = '\0';
				if (strstr((const char *)buf, term)) return num;
			}
		}
		// if we didn't get enough, wait for more
		FD_ZERO(&rfds);
		FD_SET(port_fd, &rfds);
		tv.tv_sec = sec;
		tv.tv_usec = usec;
		r = select(port_fd+1, &rfds, NULL, NULL, &tv);
		if (r < 0) return -2;		// select error
		if (r == 0) return num;		// timeout
...
int read_serial_port_timeout(unsigned char *buf, int size, const char *term, double timeout)
{
	int r, flags;

        flags = fcntl(port_fd, F_GETFL);
        fcntl(port_fd, F_SETFL, flags | O_NONBLOCK);
	r = read_serial_port_timeout_nb(buf, size, term, timeout);
        fcntl(port_fd, F_SETFL, flags);
	return r;
}
...
int set_baud(const char *baud_name)
{
	...
	port_setting.c_cflag = baud | CS8 | CREAD | CLOCAL;
